...
export function SwipeFlow() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setNewJobs(nw.map(toUiJob)); setApprovedJobs(ap.map(toUiJob)); setAppliedJobs(ad.map(toUiJob));
      setCurrentIndex(0); setDeferredIds([]); setApprovedDelta(0); setRejectedDelta(0);
    } catch (e: any) {
  ...
  // advance to the next card after the fly-off animation
  const advance = () => setTimeout(() => { setCurrentIndex((i) => i + 1); setDirection(null); }, 240);

  // Approve — optimistic: show next card instantly, persist in background.
  const handleApprove = () => {
  ...
  // Pass — remove the card immediately, ask the reason afterwards (non-blocking).
  const handlePass = () => {
    const job = currentJob; if (!job) return;
    setDirection("left");
  ...
  if (currentIndex >= reviewJobs.length) {
    return (
      <>
        ...
        <AnimatePresence>{pendingReject && <RejectReasonSheet job={pendingReject} onPick={finalizeReject} />}</AnimatePresence>
      </>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-gray-900 relative overflow-hidden">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div className="absolute w-96 h-96 bg-gradient-to-br from-indigo-500/20 to-slate-500/20 rounded-full blur-3xl"
      ...
      <div className="relative z-10 bg-gray-800/80 backdrop-blur-lg border-b border-gray-700">
        <div className="max-w-2xl mx-auto px-5 py-3">
          ...
          <div className="h-2 bg-gray-700 rounded-full overflow-hidden">
            <motion.div className="h-full bg-gradient-to-r from-indigo-500 to-indigo-700"
      ...
      <div className="relative z-10 max-w-xl mx-auto px-5 py-6 pb-10">
        <AnimatePresence mode="wait">
          {currentJob && (
            <SwipeCard key={currentJob.id} job={currentJob} direction={direction}
              onApprove={handleApprove} onPass={handlePass} onDefer={handleDefer} />
          )}
        </AnimatePresence>
      </div>
...
function SwipeCard({ job, direction, onApprove, onPass, onDefer }: any) {
  ...
  return (
    <motion.div
      ...
      drag="x" dragConstraints={{ left: 0, right: 0 }} dragElastic={0.7} onDragEnd={onDragEnd} whileDrag={{ cursor: "grabbing" }}
      className="relative">
        ...
        <div className="h-28 bg-gradient-to-br from-indigo-600 via-indigo-700 to-slate-800 relative">
          ...
          <div className="absolute bottom-4 left-5 right-5 flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 text-white/90 text-sm mt-0.5"><MapPin className="w-4 h-4" /><span className="truncate">{(job.location || "").split(",")[0]}</span></div>
            </div>
          </div>
        </div>
        <div className="p-5 space-y-5">
          ...
          {job.whyFits && (
            <div className="bg-gradient-to-br from-amber-950/50 to-orange-950/50 rounded-2xl p-4 border-2 border-amber-700">
              <div className="flex items-center gap-2 mb-2"><div className="w-7 h-7 bg-amber-500 rounded-full flex items-center justify-center"><Sparkles className="w-4 h-4 text-white" /></div><h4 className="font-bold text-amber-400 text-sm">Why this is a strong fit</h4></div>
...
function RejectReasonSheet({ job, onPick }: { job: UiJob; onPick: (k: string | null) => void }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 z-[55] flex items-end justify-center" onClick={() => onPick(null)}>
      <div className="absolute inset-0 bg-black/40" />
      <motion.div initial={{ y: 60 }} animate={{ y: 0 }} exit={{ y: 60 }} onClick={(e) => e.stopPropagation()}
        className="relative w-full max-w-xl bg-gray-900 border-t border-gray-700 rounded-t-3xl p-5 pb-7 safe-bottom">
        <div className="w-10 h-1.5 bg-gray-700 rounded-full mx-auto mb-4" />
        ...
        <div className="space-y-2.5">
          {Object.entries(REJECT_REASONS).map(([k, label]) => (
            <button key={k} onClick={() => onPick(k)} className="w-full py-3 bg-gray-800 border border-gray-700 active:bg-gray-700 text-gray-200 rounded-xl font-medium">{label}</button>
          ))}
...
function CenterState({ icon, title, action }: { icon: any; title: string; action?: { label: string; onClick: () => void } }) {
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-gray-900 flex flex-col items-center justify-center p-6 text-center">
      {icon}<p className="text-gray-300 mt-4 mb-4">{title}</p>
      {action && <button onClick={action.onClick} className="px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-xl font-medium">{action.label}</button>}
    </div>
  );
}
...
function DashboardView(p: any) {
  const { me, activeTab, setActiveTab, onBackToSwipe, approvedJobs, appliedJobs, approvedCount, rejectedCount, deferredCount, selectedJob, setSelectedJob, onOpenSettings, showSettings, onCloseSettings } = p;
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      <header className="bg-gray-800 border-b border-gray-700 safe-top">
        <div className="max-w-3xl mx-auto px-5 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={onBackToSwipe} className="px-3.5 py-2 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-xl text-sm font-medium flex items-center gap-2"><Briefcase className="w-4 h-4" />Swipe</button>
      ...
      <div className="max-w-3xl mx-auto px-5 py-5">
        ...
        {/* Tab grid: all six visible, nothing hidden off-screen */}
        <div className="grid grid-cols-3 gap-2 mb-4">
          {TABS.map(([id, label, Icon]) => {
            const active = activeTab === id;
            return (
              <button key={id} onClick={() => setActiveTab(id)}
                className={`flex flex-col items-center justify-center gap-1 py-3 rounded-xl border transition-colors ${active ? "bg-indigo-600 border-indigo-500 text-white" : "bg-gray-800 border-gray-700 text-gray-400 active:bg-gray-700"}`}>
...
function StatCard({ label, value, sub, icon }: any) {
  return (
    <div className="bg-gray-800 rounded-xl p-4 border border-gray-700">
      ...
      <div className="text-2xl font-bold text-white">{value}</div><p className="text-xs text-gray-500 mt-0.5">{sub}</p>
    </div>
  );
}
...
function ActivityTab() {
  ...
  return (
    <div className="space-y-3">
      {items.map((a) => {
        const ev = (a.event_type || "").toLowerCase(); const isSearch = ev.includes("search"); const isFail = ev.includes("fail") || ev.includes("reject");
        return (
          <div key={a.id} className="flex items-start gap-3 p-3.5 bg-gray-800 rounded-xl border border-gray-700">
            <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${isSearch ? "bg-indigo-900/40" : isFail ? "bg-red-900/40" : "bg-green-900/40"}`}>
              {isSearch ? <SearchIcon className="w-4 h-4 text-indigo-400" /> : isFail ? <XCircle className="w-4 h-4 text-red-400" /> : <CheckCircle className="w-4 h-4 text-green-400" />}
            </div>
...
function AnalyticsTab({ approvedCount, rejectedCount, deferredCount, appliedCount }: any) {
  ...
  const bar = (label: string, n: number, color: string) => (
    <div><div className="flex items-center justify-between mb-1"><span className="text-sm text-gray-400">{label}</span><span className="text-sm font-medium text-white">{n}</span></div>
      <div className="h-2 bg-gray-700 rounded-full overflow-hidden"><div className={`h-full ${color}`} style={{ width: `${totalReviewed ? (n / totalReviewed) * 100 : 0}%` }} /></div></div>
  );
  return (
    <div className="space-y-5">
      ...
      <div className="bg-gradient-to-br from-indigo-900/20 to-slate-800/40 rounded-2xl p-5 border border-indigo-800/40">
        ...
        <ul className="space-y-2 text-sm text-gray-300">
          <li className="flex items-start gap-2"><Target className="w-4 h-4 text-indigo-400 shrink-0 mt-0.5" /><span>You've reviewed {totalReviewed} job{totalReviewed !== 1 ? "s" : ""} so far.</span></li>
  ...
  return (
    <div className="flex items-center gap-3 p-3.5 bg-gray-800 rounded-xl border border-gray-700 active:border-indigo-500 cursor-pointer" onClick={() => onSelect(job)}>
      ...
      <div className="flex-1 min-w-0">
        <h4 className="font-semibold text-white truncate">{job.title}</h4><p className="text-sm text-gray-400 truncate">{job.company}</p>
      ...
      <div className="flex items-center gap-2 shrink-0">
        <div className="text-right"><div className="text-base font-bold text-indigo-400">{job.matchScore === null ? "—" : `${job.matchScore}%`}</div><div className="text-[10px] text-gray-500">match</div></div>
      ...
      <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 40, opacity: 0 }} onClick={(e) => e.stopPropagation()} className="bg-gray-900 rounded-t-3xl sm:rounded-3xl shadow-2xl max-w-2xl w-full max-h-[88vh] overflow-y-auto no-scrollbar border border-gray-700">
        ...
        <div className="p-5 space-y-5">
          <div className="flex items-start gap-4">
            <div className="w-16 h-16 bg-gradient-to-br from-indigo-900/40 to-slate-800 rounded-2xl flex items-center justify-center shrink-0"><Building2 className="w-8 h-8 text-indigo-400" /></div>
            <div className="flex-1 min-w-0"><h3 className="text-xl font-bold text-white mb-0.5">{job.title}</h3><p className="text-lg text-indigo-400 font-semibold mb-1">{job.company}</p>
              <div className="flex items-center gap-4 text-gray-400 flex-wrap">{job.location && <div className="flex items-center gap-1"><MapPin className="w-4 h-4" /><span className="text-sm">{job.location}</span></div>}{job.timeAgo && <div className="flex items-center gap-1"><Clock className="w-4 h-4" /><span className="text-sm">{job.timeAgo}</span></div>}</div>
            </div>
          </div>
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-2 mb-2">
        {value.map((t) => (
          <span key={t} className="inline-flex items-center gap-1.5 pl-3 pr-2 py-1.5 bg-indigo-600/20 border border-indigo-600/50 text-indigo-200 rounded-full text-sm">
            {t}<button onClick={() => onChange(value.filter((x) => x !== t))} className="w-4 h-4 flex items-center justify-center rounded-full bg-indigo-500/40 active:bg-indigo-500"><X className="w-3 h-3" /></button>
          </span>
        ))}
      ...
      <div className="flex gap-2">
        <input value={draft} onChange={(e) => setDraft(e.target.value)}
          ...
          placeholder="e.g. VP Product" className="flex-1 px-4 py-2.5 border border-gray-700 rounded-xl bg-gray-800 text-white text-sm" />
        <button onClick={add} className="px-3.5 bg-indigo-600 active:bg-indigo-700 text-white rounded-xl flex items-center justify-center"><Plus className="w-5 h-5" /></button>
      </div>
...
function SettingsModal({ me, onClose }: { me: Me & any; onClose: () => void }) {
  ...
  const enableNotifs = async () => {
    setPushMsg("Enabling…"); const r = await enablePush(); setPerm(pushState());
  ...
  const save = async () => {
    ...
    try {
      await api.saveProfile({ phone, job_titles: titles });
  ...
  const onCvPick = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch("/api/upload-cv", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ filename: file.name, data: btoa(bin) }) });
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center sm:p-6" onClick={onClose}>
      <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 40, opacity: 0 }} onClick={(e) => e.stopPropagation()} className="bg-gray-900 rounded-t-3xl sm:rounded-3xl shadow-2xl max-w-xl w-full max-h-[88vh] overflow-y-auto no-scrollbar border border-gray-700">
        ...
        <div className="p-5 space-y-6">
          ...
          <div>
            <h3 className="font-semibold text-white mb-3 flex items-center gap-2"><Briefcase className="w-5 h-5 text-indigo-400" />Job Titles</h3>
          ...
          <div>
            ...
            <label className="block border-2 border-dashed border-gray-700 rounded-xl p-6 text-center active:border-indigo-500 cursor-pointer bg-gray-800/50">
              <input type="file" accept=".pdf,.doc,.docx" className="hidden" onChange={onCvPick} />
              <p className="text-gray-400 text-sm mb-1">Tap to upload</p><p className="text-xs text-gray-500">PDF or DOCX, max 5MB</p>
              {cvName && <p className="text-xs text-green-400 mt-2">✓ {cvName}</p>}{cvMsg && <p className="text-xs text-gray-400 mt-1">{cvMsg}</p>}
            </label>
          </div>
          <div>
            ...
            {perm === "unsupported" ? (<p className="text-sm text-gray-400">This browser doesn't support push notifications.</p>) : (
              <div className="space-y-2">
                <button onClick={enableNotifs} disabled={perm === "granted"} className="w-full py-3 bg-gray-800 border border-gray-700 text-gray-200 rounded-xl font-medium disabled:opacity-60">{perm === "granted" ? "✓ Notifications enabled" : "Enable push notifications"}</button>
                {perm === "granted" && <button onClick={sendTest} className="w-full py-2.5 bg-gray-700 active:bg-gray-600 text-gray-200 rounded-xl text-sm font-medium">Send test notification</button>}
          ...
          <button onClick={save} disabled={saving} className="w-full py-4 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-xl font-semibold disabled:opacity-60 flex items-center justify-center gap-2">
...
function Field({ label, sub, children }: any) {
  return (<div className="flex items-center justify-between gap-3 p-3.5 bg-gray-800 rounded-xl border border-gray-700"><div className="min-w-0"><p className="font-medium text-white text-sm">{label}</p><p className="text-xs text-gray-400">{sub}</p></div>{children}</div>);
}

function Select({ value, onChange, options, fmt }: { value: string; onChange: (v: string) => void; options: number[]; fmt: (n: number) => string }) {
  return (<select value={value} onChange={(e) => onChange(e.target.value)} className="px-3 py-2 border border-gray-700 rounded-lg bg-gray-700 text-white text-sm shrink-0">{options.map((o) => <option key={o} value={o}>{fmt(o)}</option>)}</select>);
}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-gray-900 flex items-center justify-center p-6">
      <motion.div initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center max-w-md w-full">
        ...
        <div className="space-y-3">
          <button onClick={onViewDashboard} className="w-full py-4 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-xl font-semibold flex items-center justify-center gap-2"><LayoutGrid className="w-5 h-5" />View Dashboard</button>
